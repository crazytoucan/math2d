...
export function polyline2GetNearestT(poly: IPolyline2, point: IVec2) {
  ...
  for (let i = 0; i < len; i++) {
    const vert0 = vec2Reset(poly[2 * i], poly[2 * i + 1], TMP_VEC0);
    const vert1 = vec2Reset(poly[2 * i], poly[2 * i + 1], TMP_VEC1);
